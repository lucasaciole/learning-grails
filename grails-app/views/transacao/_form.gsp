...
<div class="fieldcontain ${hasErrors(bean: transacaoInstance, field: 'contaCliente', 'error')} required">
	...
	<g:select id="contaCliente" name="contaCliente.id" from="${br.ufscar.dc.dsw.ContaCliente.list()}" optionKey="id" required="" value="${transacaoInstance?.contaCliente?.id}" class="many-to-one"/>

</div>

<div class="fieldcontain ${hasErrors(bean: transacaoInstance, field: 'caixaEletronico', 'error')} required">
	<label for="caixaEletronico">
		<g:message code="transacao.caixaEletronico.label" default="Caixa Eletronico" />
		<span class="required-indicator">*</span>
	</label>
	<g:select id="caixaEletronico" name="caixaEletronico.id" from="${br.ufscar.dc.dsw.CaixaEletronico.list()}" optionKey="id" required="" value="${transacaoInstance?.caixaEletronico?.id}" class="many-to-one"/>
...
<div class="fieldcontain ${hasErrors(bean: transacaoInstance, field: 'tipo', 'error')} required">
	...
	<g:select name="tipo" from="${transacaoInstance.constraints.tipo.inList}" required="" value="${transacaoInstance?.tipo}" valueMessagePrefix="transacao.tipo"/>
